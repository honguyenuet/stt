...
function createSocialRegistrationToken(profile) {
  ...
  const providerUserId = String(profile.providerUserId || "").trim().slice(0, 255);
...
function createSocialRegistrationUrl(frontendUrl, profile) {
  const url = new URL("/register", `${String(frontendUrl || "").replace(/\/$/, "")}/`);
  const provider = cleanProvider(profile.provider);
  const email = cleanEmail(profile.email);
  ...
  try {
    ...
    if (!createIfMissing) {
      const error = createSocialIdentityError(
        `${provider}_account_not_registered`,
        "Tài khoản mạng xã hội này chưa được đăng ký.",
        404,
      );
