...
vi.mock("@tanstack/react-router", async (importOriginal) => {
  ...
  return {
    ...
    }) => select({ location: { pathname: "/record" } }),
  };
});

import { VbeeStyleFooter } from "./record";

describe("VbeeStyleFooter", () => {
  it("keeps the recording page footer in its compact original style", () => {
    const markup = renderToStaticMarkup(<VbeeStyleFooter />);

    expect(markup).toContain("border-t border-border bg-white");
