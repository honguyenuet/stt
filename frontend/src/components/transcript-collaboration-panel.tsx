  ...
  return (
    <div className="space-y-4 text-sm">
      ...
      <div className="rounded-lg border border-border p-3">
        ...
        <div className="mt-2 flex gap-2"><input aria-label="Nội dung bình luận" value={comment} maxLength={2000} onChange={(event) => setComment(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void addComment(); }} placeholder="Bình luận tại vị trí đang phát…" className="h-9 min-w-0 flex-1 rounded-lg border border-border px-2 text-xs" /><button type="button" aria-label="Gửi bình luận" onClick={() => void addComment()} disabled={!comment.trim() || Boolean(busy)} className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary text-primary-foreground disabled:opacity-40"><Send className="h-4 w-4" /></button></div>
      </div>
    </div>
  );
}
